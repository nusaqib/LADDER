...
\title{LADDER: AN LLM-DRIVEN, HUMAN-OVERSEEN LOOP FOR VENDOR-NEUTRAL GENERATION OF VERIFIED PLC PROGRAMS}
...
LADDER organizes PLC software development as an \emph{LLM-driven loop
under human oversight}: an assistant (any large language model)
interviews the engineer for the ground truth only they have --- signal
senses, safety philosophy, acceptance stories --- and drafts the
design; a deterministic, LLM-free core judges every draft; the
engineer reviews diffs and signs off at defined gates. What the
assistant drafts is never vendor code but a declarative,
schema-validated intermediate representation (IR) of domain elements
--- interlocks, redundant (1oo2) inputs, search/arming chains,
annunciator groups, timers, state machines, PID loops --- from which
...
repositories with pinned toolchains and continuous integration, and
the pipeline remains fully operable with no model at all. We describe
the loop, the IR, the lowering and
...
\section{The Authoring Loop}

LADDER's central organizing idea is a three-actor loop with a strict
segregation of authority. The \emph{human} is the only source of
ground truth --- what the machine does and must never do, every
signal's meaning and Boolean sense, the safety philosophy (what trips
what, what latches, who may reset, redundancy), the acceptance stories
that define done, and the hardware reality --- and the only authority
that lands a change, at four small review gates: intake sign-off,
behavior sign-off (the scenarios), proof review, and the deploy diff.
The \emph{assistant} (any LLM; the contract is plain JSON Schema and
YAML) conducts the intake interview, then drafts the design map, the
IR, and the scenario suite --- mechanical transcription of the human's
decisions into constrained, diffable text, which is precisely the task
current models do well and reviews catch cheaply. The \emph{machine} is
deterministic and LLM-free: validation and lint with machine-actionable
issue codes, lowering, simulation, formal proof, artifact and vendor
builds. Its outputs are the feedback that drives the assistant's next
draft, so the loop converges on evidence rather than on plausibility;
and because the gates are identical for human-typed and model-drafted
YAML, the assistant is a productivity multiplier, never a trusted
component. Removing the assistant leaves a complete, manually operable
toolchain --- the loop degrades gracefully to classical engineering.
...
version 0.2 with a semver/RFC process toward a frozen 1.0. Practices
adopted from the surveyed ecosystem are already landed: PLCverif-style
requirement patterns~\cite{plcverif-status}, counterexamples replayed
as runnable simulator scenarios, mutation testing that scores the
acceptance suite by fault injection (and found real gaps in our own
starter suite), a packaged backend-conformance corpus, semantic design
diffs, behavior-preserving L5X reverse adoption, and EPICS interface
records generated from the same IR. Remaining directions include
TwinCAT reverse adoption and emulator-in-the-loop stages. Generated
logic must always be reviewed by qualified controls engineers; LADDER
is engineering automation and evidence generation, not certification.
